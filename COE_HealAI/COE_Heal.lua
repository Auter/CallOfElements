...
function COE_Heal:DetermineHealTarget(useHealComm, isSpamMode)
	...
	local ignorePets = true;  -- Default: ignore pets unless explicitly turned off
	local emergencyThreshold = 0.55;
	local groupPrio = nil;
	...
	if COE_Saved then
		...
		-- FIX: HB_IgnorePets == 1 means "ignore pets ON", so ignorePets should be true
		-- HB_IgnorePets == 0 or nil means "ignore pets OFF", so ignorePets should be false
		if COE_Saved.HB_IgnorePets ~= nil then
	...
	-- Debug output for pet settings
	COE:DebugMessage("Pet handling: ignorePets=" .. tostring(ignorePets) .. 
		" (HB_IgnorePets=" .. tostring(COE_Saved and COE_Saved.HB_IgnorePets or "nil") .. ")");
	...
	local function AddCandidate(unit)
		...
		-- Check pet filter
		if ignorePets and IsPet(unit) then 
			COE:DebugMessage("Candidate rejected (ignorePets filter): " .. (UnitName(unit) or unit) .. 
				" (unitID=" .. unit .. ")");
			return;
		end
		...
		-- Debug output for candidate addition
		local isPetUnit = string.find(unit, "pet") ~= nil;
		COE:DebugMessage("Candidate added: " .. (UnitName(unit) or unit) .. 
			" (unitID=" .. unit .. ", HP=" .. math.floor(curRatio * 100) .. "%" ..
			(isTank and ", TANK" or "") ..
			(isPetUnit and ", PET" or "") .. ")");
	...
	if not ignorePets then
		if UnitExists("pet") then
			COE:DebugMessage("Checking player pet: " .. (UnitName("pet") or "pet") .. 
				", HP=" .. math.floor((UnitHealth("pet") / math.max(1, UnitHealthMax("pet"))) * 100) .. "%");
		end
		AddCandidate("pet");
	else
		if UnitExists("pet") then
			COE:DebugMessage("Skipping player pet (ignorePets=true)");
		end
	end
	...
	for i = 1, GetNumPartyMembers() do
		...
		if not ignorePets then
			...
			if UnitExists(petUnit) then
				COE:DebugMessage("Checking party pet: " .. (UnitName(petUnit) or petUnit) .. 
					", HP=" .. math.floor((UnitHealth(petUnit) / math.max(1, UnitHealthMax(petUnit))) * 100) .. "%");
			end
			AddCandidate(petUnit);
		end
	end
	...
	for i = 1, GetNumRaidMembers() do
		...
		if not ignorePets then
			...
			if UnitExists(petUnit) then
				COE:DebugMessage("Checking raid pet: " .. (UnitName(petUnit) or petUnit) .. 
					", HP=" .. math.floor((UnitHealth(petUnit) / math.max(1, UnitHealthMax(petUnit))) * 100) .. "%");
			end
			AddCandidate(petUnit);
		end
			...
			if dispelResult then
				...
				if spellID then
					...
					if start == 0 or duration == 0 then
						...
						-- Enhanced debug output for dispel
						COE:DebugMessage("Dispel casting: spell=" .. spellName .. 
							" on " .. (UnitName(dispelResult.target) or dispelResult.target) ..
							" (unitID=" .. dispelResult.target .. ")");
						
						-- In Vanilla 1.12, we must TARGET the unit BEFORE casting helpful spells
						-- Using CastSpell() + SpellTargetUnit() does NOT work for helpful spells
						-- This is the same pattern used for healing
						TargetUnit(dispelResult.target);
...
function COE_Heal:HasDebuff(unit)
	...
	while true do
		...
		-- Debug output for dispel scanning
		COE:DebugMessage("Dispel scan: " .. (UnitName(unit) or unit) .. 
			" (unitID=" .. unit .. ") debuff #" .. i .. 
			": type=" .. (debuffType or "nil") .. 
			", texture=" .. (texture or "nil"));
		
		if debuffType == "Poison" and dispelPoison then
			COE:DebugMessage("  -> DISPELLABLE (Poison)");
			return "Poison";
		elseif debuffType == "Disease" and dispelDisease then
			COE:DebugMessage("  -> DISPELLABLE (Disease)");
			return "Disease";
		end
