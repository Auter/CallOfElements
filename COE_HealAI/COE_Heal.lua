...
function COE_Heal:Heal( type )
	...
	COE:DebugMessage( "Attempting to heal " .. target.unit .. " at " .. math.floor(target.ratio * 100) .. 
		"% (missing: " .. deficit .. ", for rank: " .. math.floor(deficitForRank) .. ")" );
	...
	if target.inTankEmergency then
		...
		id, spell, rank = COE_Heal:DetermineBattleSpell( deficit );
	elseif target.ratio < emergencyThreshold then
		-- EMERGENCY: Use fast heal (Lesser Healing Wave / BattleHeal)
		-- Use RAW deficit for emergencies - don't under-heal
		...
		-- Here we use the clamped deficit if in RANK_ADJUST mode
		if type == "best" then
			id, spell, rank = COE_Heal:DetermineBestSpell( COE.HealData.Sorted, deficitForRank );
		else
			id, spell, rank = COE_Heal:DetermineBattleSpell( deficitForRank );
		end
	end
...
--[[ ----------------------------------------------------------------
	...
	PURPOSE: Determines the spell to be used to heal the given
		health deficit. Uses smart rank selection:
		1. Find the lowest rank that can fully cover the deficit
		2. If no rank covers it, use highest rank we can afford
...
function COE_Heal:DetermineBestSpell( spelltable, healamount )
	...
	-- Helper function to find best spell from a table
	-- Smart healing: find lowest rank that covers deficit, or max rank if nothing covers it
	local function FindBestFromTable(tbl)
		...
		for i = 1, table.getn(tbl) do
			local spell = tbl[i];
			if spell.AvgAmount >= healamount then
	...
	if useLesser and hasLesser and not useWave then
		...
		bestSpell = FindBestFromTable(lesserTable);
	elseif useWave and hasWave and not useLesser then
		-- ONLY Wave
		bestSpell = FindBestFromTable(waveTable);
	else
