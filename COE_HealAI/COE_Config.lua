...
--[[ ----------------------------------------------------------------
	...
	NOTE: Only initialize if not already set, to prevent double-load
	from COE_Config.xml and COE_ConfigTemplates.xml from wiping data
-------------------------------------------------------------------]]
if( not COE_Config["Options"] ) then
	COE_Config["Options"] = {};
end

-- ============================================================
-- HealAI UI (Phase 2): simple Saved settings in COE_Saved
-- ============================================================
function COE_Config:HB_InitDefaults()
	-- Ensure COE_Saved exists (may be called before VARIABLES_LOADED)
	...
	if (COE_Saved["HB_UseTargetTarget"] == nil) then COE_Saved["HB_UseTargetTarget"] = 1 end
end

function COE_Config:HB_UI_Refresh()
	if (not COE_Saved) then COE_Saved = {} end
	if (not COE_HB_Enable) then return end

	COE_HB_Enable:SetChecked(COE_Saved["HB_Enable"] == 1)

	if (COE_HB_TankNameEdit) then
		COE_HB_TankNameEdit:SetText(COE_Saved["HB_TankName"] or "")
	end
	...
	if (COE_HB_UseTargetTarget) then
		COE_HB_UseTargetTarget:SetChecked(COE_Saved["HB_UseTargetTarget"] == 1)
	end
end

function COE_Config:HB_UI_Save()
	if (not COE_Saved) then return end
	if (not COE_HB_Enable) then return end

	COE_Saved["HB_Enable"] = (COE_HB_Enable:GetChecked() and 1 or 0)

	if (COE_HB_TankNameEdit) then
		local t = COE_HB_TankNameEdit:GetText() or ""
		COE_Saved["HB_TankName"] = string.gsub(t, "^%s*(.-)%s*$", "%1")
	end

	if (COE_HB_TopUpSlider) then
		local v = math.floor((COE_HB_TopUpSlider:GetValue() or 85) + 0.5)
		if (v < 50) then v = 50 end
		if (v > 100) then v = 100 end
	...
	if (COE_HB_UseTargetTarget) then
		COE_Saved["HB_UseTargetTarget"] = (COE_HB_UseTargetTarget:GetChecked() and 1 or 0)
	end
end

function COE_Config:HB_SetTankFromTarget()
	if (not COE_Saved) then return end
	local name = UnitName("target")
	if (name and name ~= "") then
		COE_Saved["HB_TankName"] = name
		if (COE_HB_TankNameEdit) then
			COE_HB_TankNameEdit:SetText(name)
		end
	end
end
...
--[[ ----------------------------------------------------------------
	...
	IMPORTANT: Do NOT initialize here at file load time!
	SavedVariables are restored by WoW AFTER Lua files are parsed.
	Initialization must happen in VARIABLES_LOADED event handler.
	
	This table stores the DEFAULT values to apply to missing keys.
-------------------------------------------------------------------]]
COE_Config.SavedDefaults = {
	[COEOPT_ENABLETOTEMBAR] = 1,
	[COEOPT_HIDEBACKDROP] = 1,
	[COEOPT_ENABLETIMERS] = 1,
	[COEOPT_TIMERNOTIFICATIONS] = 1,
...
--[[ ----------------------------------------------------------------
	METHOD: COE_Config:InitSavedVariables
	
	PURPOSE: Called AFTER VARIABLES_LOADED to apply defaults to
	any missing keys. This must NOT be called at file load time.
-------------------------------------------------------------------]]
function COE_Config:InitSavedVariables()
	-- Create table if WoW didn't restore one (first time install)
	if not COE_Saved then
		COE_Saved = {}
	end
	
	-- Apply defaults only for missing keys
	for key, default in pairs(COE_Config.SavedDefaults) do
		if COE_Saved[key] == nil then
			COE_Saved[key] = default
		end
	end
end
...
--[[ ----------------------------------------------------------------
	...
	IMPORTANT: Initialization moved to COE_Config:InitSavedVariables()
	to run AFTER VARIABLES_LOADED event.
-------------------------------------------------------------------]]

--[[ ----------------------------------------------------------------
	METHOD: COE_Config:InitTotemSets
	
	PURPOSE: Initialize totem set SavedVariables AFTER VARIABLES_LOADED
-------------------------------------------------------------------]]
function COE_Config:InitTotemSets()
	if not COE_SavedTotemSets then
		COE_SavedTotemSets = { 
	...
	if not COE_CustomTotemSets then
		COE_CustomTotemSets = { n = 0 }
	end
	
	if not COE_DisplayedTotems then
		COE_DisplayedTotems = { }
	end
	
	if not COE_TotemBars then
		COE_TotemBars = {
...
function COE_Config:OnFrameLoad()
	...
	COE_Config:RegisterOption( COEOPT_ENABLEFORCESETS, 'check', COEOptionEnableForceSets, COE_Config:GetSaved( COEOPT_ENABLEFORCESETS ) );
	COE_Config:RegisterOption( COEOPT_ENABLEDISTANCECHECK, 'check', COEOptionEnableDistanceCheck, COE_Config:GetSaved( COEOPT_ENABLEDISTANCECHECK ) );
...
function COE_Config:OnCheckBoxShow()
	...
	-- call function
	-- --------------
	if( COE_Config.Options[this:GetID()] and COE_Config.Options[this:GetID()].Func ) then
...
function COE_Config:ActivateSet( index )
	...
	-- update drop down
	-- -----------------
	if COE_Config.Options[COEOPT_ACTIVESET] and COE_Config.Options[COEOPT_ACTIVESET].InitFunc then
		UIDropDownMenu_Initialize( COE_OptionActiveSetCB, COE_Config.Options[COEOPT_ACTIVESET].InitFunc );
	else
		COE:DebugMessage("Warning: COEOPT_ACTIVESET not registered yet");
	end
	...
	if set and set.CastOrder then
		local i;
		for i = 1, 4 do
			local textObj = getglobal( "COE_OptionCastOrder" .. i .. "Text" );
			if textObj then
				textObj:SetText( set.CastOrder[i] or "" );
			end
		end
	else
		COE:DebugMessage("Warning: Totem set data missing for index " .. tostring(index));
	end
...
function COE_Config:SetSetButtonStates()
	...
	-- set button states
	-- ------------------
	local selectedValue = UIDropDownMenu_GetSelectedValue( COE_OptionActiveSetCB );
	if( selectedValue == nil ) then
		-- Dropdown not initialized yet, default to disabled
		if COE_OptionDeleteSet then
			COE_OptionDeleteSet:Disable();
		end
		return;
	end
	
	if( selectedValue <= COESET_DEFAULT ) then
...
function COE_Config:DeleteTotemSet()
	...
	if( set == nil or set <= COESET_DEFAULT ) then
		return;
	end
...
function COE_Config:GetSaved( id )
	...
	-- Check COE_Saved exists and has this key
	if COE_Saved and COE_Saved[id] ~= nil then
		return COE_Saved[id];
